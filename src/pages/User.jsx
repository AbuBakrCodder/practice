...
function User() {
    ...
    const { data: users, error, loading, formData, createUser, deleteUser } = useFetch(api)
                ...
                {users && users.map((user) => {
                    return (
                        <div key={user.id} className="bg-white text-black flex items-center justify-center flex-col p-5 gap-3 w-[300px] h-[200px] rounded-4xl relative">
                            ...
                            <button className="absolute top-2 right-5 cursor-pointer"><i class="fa-solid fa-user-pen"></i></button>
                        </div>
                    )
                })}
            </div>
